...
void zipFileWriter(int* num, node* tree, int s, int e, string fileName);   //reads a file and makes its zip file
void fileExtractor(node* tree, int x, FILE* input, string fileName);  //writes the extracted file
...
int main(){
    ...
    if(selection==1){
        ...
        treeBuilder(num, tree, x, s, e);  //making the tree

        zipFileWriter(num, tree, s, e, fileName);   //writing zip file
    }
        ...
        if((input=fopen(fName, "r"))==NULL){ //opening the file
            ...
            fileExtractor(tree, x, input, fileName);
...
void charCounter(int* num, int s, int e, string fileName){
    ...
    if((input=fopen(fName, "r"))==NULL){ //opening the file, if file is not found return
        printf("File is not found!!\n");
        return ;
    }
...
void zipFileWriter(int* num, node* tree,int s, int e, string fileName){
    ...
    stack<bool> mystack;
    ch=fgetc(input);
    while(!feof(input)){ //reading the file once more & writing in the output file
        //cout << ch <<" "; ///test
...
void fileExtractor(node* tree, int x, FILE *input, string fileName){
    FILE *output;
